...
int main(void) {
    ...
    char dealCards = '\0'; //This is the variable that waits for the user input if they want to deal the cards
    char contPlay = 'y';
    ...
    scanf("%d", &shuffleOrNo);
    while ((shuffleOrNo != 1) && (shuffleOrNo != 2)) {
    ...
    printf("Press any letter or number key to deal cards!\n");
    scanf(" %c", &dealCards);
...
void mainGame(card* deck, int* curCard, int* discardPileIndex, card** player, card** playerOther, int playerNumber, int* drawCards) {
    ...
    char newColor[7]; //These are the colors of the cards
    ...
    if (playerAct != -1) { //This is if the player wants to play a card
        if (temp->value == 7) { //This is for extra credit
            ...
            *drawCards += 2;
            ...
            scanf(" %s", newColor);
            strncat(newColor, "\0", 1);
            strcpy(temp->color, newColor);
            printf("%s %s", temp->action, temp->color);
            printf("\nPlayer %d placed: %s %s\n", playerNumber, temp->action, temp->color);
            playerSubtractCard(player, deck, playerAct, discardPileIndex);
...
void ReadDeck(FILE* inp, card* deck) { //This function is used to read the ordered deck from the file into the deck variable
    ...
    while (!feof(inp)) {
        fscanf(inp, "%s %d %s", deck[i].color, &deck[i].value, deck[i].action); //This scans each value into each index of deck
        i++; //incriment i
    }
    fclose(inp); //This closes the input file
}//The purpose of this function is if the players want to load in their own deck that is already shuffled or created they can do that. This function reads the whole file and then it scans each card and its attributes into an array. Then the function closes the file to ensure that the file isn't staying open and using memory.
...
void initDeck(card* deck) { //if players don't want to load deck then this deck will be initialized
    ...
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 14; j++) {
            if (j < 10) {
                strcpy(deck[count].color, color[i]);
                deck[count].value = j;
                ...
                strcpy(deck[count].color, color[i]);
                deck[count].value = j;
                strcpy(deck[count].action, "skip");
            }
            else if (j == 11) {
                strcpy(deck[count].color, color[i]);
                deck[count].value = j;
                strcpy(deck[count].action, "reverse");
            }
            else if (j == 12) {
                strcpy(deck[count].color, color[i]);
                deck[count].value = j;
                strcpy(deck[count].action, "plus two");
            }
            else {
                strcpy(deck[count].color, "");
                deck[count].value = j;
                strcpy(deck[count].action, "wild");
            }
            count++;
        }
    ...
    for (i = 0; i < 4; i++) {
        for (j = 1; j < 14; j++) {
            if (j < 10) {
                ...
                strcpy(deck[count].color, "");
                deck[count].value = j;
                strcpy(deck[count].action, "wild plus four");
            }
            count++;
        }
...
void playerHandinit(card** player1, card** player2, card* deck, int numPlayers, int* curCard, int* discardIndex) { //This is the player hand initialize function
    ...
    *player2 = (card*)malloc(sizeof(card));  //This creates the head pointer for player 2
    ...
    strcpy(temp1->color, deck[*curCard].color);
    temp1->value = deck[*curCard].value;
    strcpy(temp1->action, deck[*curCard].action); //The above 3 lines give player 1 their first card

    (*curCard)++;

    strcpy(temp2->color, deck[*curCard].color);
    temp2->value = deck[*curCard].value;
    strcpy(temp2->action, deck[*curCard].action); //The above 3 lines give player 1 their first card
    ...
    for (*curCard = 2; *curCard < 14; (*curCard)++) {
        if (*curCard % 2 != 0) {
            ...
            strcpy(temp1->color, deck[*curCard].color);
            temp1->value = deck[*curCard].value;
            strcpy(temp1->action, deck[*curCard].action);
            temp1->pt = 0;
            ...
            strcpy(temp2->color, deck[*curCard].color);
            temp2->value = deck[*curCard].value;
            strcpy(temp2->action, deck[*curCard].action);
            temp2->pt = 0;
            //This goes through a loop adding 6 more cards to the players hand in while making it a linked list in an alternating order
        }
    }
    ...
    strcpy(deck[*discardIndex].color, deck[*curCard].color);
    deck[*discardIndex].value = deck[*curCard].value;
    strcpy(deck[*discardIndex].action, deck[*curCard].action);
    (*discardIndex)++;
    (*curCard)++;
...
void playerAddCard(int* discardPileIndex, int* curCard, card* deck, card** head) {
    ...
    strcpy(newCard->color, deck[*curCard].color);
    strcpy(newCard->action, deck[*curCard].action);
    newCard->pt = *head;
    (*curCard)++;
